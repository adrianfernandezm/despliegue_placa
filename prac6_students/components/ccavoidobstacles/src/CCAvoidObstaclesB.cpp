	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (Start):
				...
				break;
			//To Choice Point CheeckObstacles
			case (CheeckObstacles):
				...
				if( GObsDetected() )
				{
					...
					//Branch taken is CheeckObstacles_ObstDetected
					edroomCurrentTrans.localId =
						CheeckObstacles_ObstDetected;

					//Next State is Ready
					edroomNextState = Ready;
				 } 
				//Default Branch NoObstacles
				else
				{

					//Branch taken is CheeckObstacles_NoObstacles
					edroomCurrentTrans.localId =
						CheeckObstacles_NoObstacles;
		...
		switch(Msg->signal)
		{
			...
			case (EDROOMSignalTimeout): 
				 ...
				 if (*Msg->GetPInterface() == Timer)
				{
					...
					//Next transition is  CheeckObstacles
					edroomCurrentTrans.localId = CheeckObstacles;
					edroomCurrentTrans.distanceToContext = 0 ;
					edroomValidMsg=true;
				 }
